...
采用媒体收件箱式桌面界面：粘贴后先解析为可勾选的作品列表，再统一选择清晰度、输出内容和保存位置。首次启用语音识别或中文翻译时，会明确显示模型大小与保存目录，只有用户确认后才下载。1.1.1 起支持应用内检查更新、签名校验、一键安装并自动重启；1.1.2 增加可见下载进度、已安装语音模型切换；1.1.3 修复 TikTok 下载长时间等待与隐藏 Edge 残留；1.1.4 修复中文、emoji 下载目录造成的语音识别失败，并保留文案和翻译选项、显示完整处理结果。
...
- TikTok 公开作品无需登录；必要时通过 Windows 自带 Edge 建立一次性匿名播放会话，任务结束后立即清理。
