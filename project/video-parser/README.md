...
不想维护服务器时可使用轻量的 [影链工坊 Desktop](desktop/README.md)：无需安装浏览器助手，支持单条、多链接批量和博主主页下载；粘贴后会先显示可勾选的作品列表，再把视频、封面、平台文案、字幕、AI 语音识别与中文翻译统一保存到用户选择的目录。语音和翻译模型首次使用时会先显示大小和保存位置，只有用户确认后才下载。
